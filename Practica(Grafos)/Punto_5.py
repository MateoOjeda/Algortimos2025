...
# IMPORTANTE PARA NO OLVIDARME:
# La clase UTILIZADA agrega arista inversa solo cuando is_directed=True.
# Por eso, para tener GRAFO NO DIRIGIDO y me rompa todo debe crear:
...
# =====================================================
# === CARGA DEL GRAFO DE LA RED SEGÚN LA IMAGEN ========
# =====================================================

# IMPORTANTE:
# La clase que usted me dio tiene un comportamiento especial:
# - Si is_directed=True → insert_edge agrega también la arista inversa.
# Esto significa que, para simular un GRAFO NO DIRIGIDO (que es lo que pide el ejercicio),
# debemos crear el grafo como is_directed=True.
nodos = [
...
for x in nodos:
    g.insert_vertex(x)      # crea cada vértice en el grafo

# -----------------------------------------------------
# (a) ASIGNAR TIPOS
# Cada nodo debe almacenar su tipo:
# pc, notebook, servidor, router, switch, impresora
# Esto se guarda en v.other_values según tu clase.
...
for v in g:
    print(v.value, "=>", v.other_values)

# -----------------------------------------------------
# b. BARRIDO DFS y BFS DESDE LAS NOTEBOOKS
# Red Hat, Debian, Arch
...
for start in inicios:
    ...
    g.deep_sweep(start)       # recorrido en profundidad usando tu método

    print("- B) BFS desde", start)
    g.amplitude_sweep(start)  # recorrido en amplitud usando tu método

# -----------------------------------------------------
#c. camino corto asi la impresora
# desde: Manjaro, Red Hat, Fedora
# Utiliza dijkstra() de tu clase

print("- C) Camino minimo hacia Impresora")
...
for orig in fuentes:
    ...
    # reconstrucción manual del camino más corto
    camino = []
    actual = 'Impresora'
...
# -----------------------------------------------------
# d. ÁRBOL DE EXPANSIÓN MÍNIMA (MST)
# usando kruskal() ya implementado en tu clase

print("- D) Arbol de expansión minima (Kruskal)")
print(g.kruskal('Router 1'))   # la clase requiere pasar un vértice inicial

# -----------------------------------------------------
# e. PC más cercana al servidor Guaraní
# (solo PCs, no notebooks)

print("- E) PC más cercana a Guarani")
